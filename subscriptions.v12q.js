                        ...
                        if (data != null && data.customer != null && data.card != null) {
                            if (data.has_subscription) {
                                ...
                                if (data.is_subscription_paused) {
                                    ...
                                    $('#modal-1 #modal-1-content').html('<p>You have an existing subscription with us. Please log in to <a style="color: #005ac1;text-decoration: none;" href="https://app.clearestimates.com"> Clearestimates.com</a> and start estimating.<br/>' + 
                                        'If you haven\'t set your password yet, please contact customer service using our chatbot, accessible from the bottom right corner of your screen.</p > ');
                                }
        ...
        cbInstance.load("components").then(() => {
            ...
            $("#checkout-form").off("submit").on("submit", function(event) {  // Submitting the checkout form
                ...
                if ($('#customer_id').val() !== "" && $('#payment_source_id').val() !== "") { //It's for an existing customer and the customer already has a valid card on file
                    ...
                    objSubscription.CouponIds = $.trim(data.coupon) !== "" ? [$.trim(data.coupon)] : [];
                    ...
                    cardComponent.tokenize(user).then(response => {
                        ...
                        objSubscription = {
                            ...
                            CustomerPhone: $.trim(data.phone) !== "" ? $.trim(data.phone) : null,
                            BillingAddressFirstName: data.first_name,
                            BillingAddressLastName: data.last_name,
                            BillingAddressLine1: $.trim(data.address),
                            BillingAddressLine2: $.trim(data.address2) !== "" ? $.trim(data.address2) : null,
                            BillingAddressCity: $.trim(data.city),
                            BillingAddressState: data.state,
                            BillingAddressZip: $.trim(data.zip),
                            BillingAddressCountry: data.country,
                            AutoCollection: AutoCollectionEnum.On,
                            CustomerCompany: $.trim(data.company_name) !== "" ? $.trim(data.company_name) : null,
                ...
                $.post(cbRegApiUrl + "createsubscription", objSubscription, function (resp) {
                    ...
                    if (resp.success && resp.subscription_id != null && resp.subscription_id !== '') {
                        $("#error").hide();
                        window.location.href = cedomain + "/subscribe/success?registrationtoken=" + resp.subscription_id + "&email=" + encodeURIComponent($.trim(data.email));
                    }
                ...
                $('#btnSavePayment').off('click').on('click', function (event) { //Saving new card 
                    ...
                    if (formData.first_name == '' || formData.last_name == '' || formData.address == '' || formData.city == '' || formData.state == '' || formData.zip == '') {
                        ...
                        if (data.customer.Id != null && data.customer.Id !== "") {
                            ...
                            //Update the customer billing info if it has been changed
                            var billingInfoObj = {
                            ...
                            var cbUpdateBillingApiUrl = cbRegApiUrl + "updatecustomerbillinginfo";
