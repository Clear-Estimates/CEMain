...
Webflow.push(function () {
    ...
    var cbRegApiUrl = "https://regapi.clearestimates.com/api/chargebee/"; //"https://reg.clearestimates.com/api/chargebee/";//CHANGE
    ...
    $(document).ready(function () {
        ...
        var hasCustomerId = customer_id != null && customer_id != '';

        //This if block is for Resubscription-standard-monthly page only
        if (hasCustomerId) {
            var cbCustomerByIdApiUrl = cbRegApiUrl + 'getcustomerbyid/' + customer_id;
            var planId = urlParams.get('plan_id').trim();
            var planName = decodeURIComponent(urlParams.get('plan_name')).trim();
            if (planId != '') {
                plan_id = planId;
            }
            if (planName != '') {
                $('.checkout_component .heading-large').html('Checkout - ' + planName);
            }
            console.log('cbCustomerByIdApiUrl:' + cbCustomerByIdApiUrl);
            $.get(cbCustomerByIdApiUrl, function (data) {
                console.log('cbCustomerByIdApiUrl Response Data :' + JSON.stringify(data));
                if (data != null) {
                    if (data.has_subscription) {
                        MicroModal.init();
                        $('#modal-1 #modal-1-content').html('<p>You have an existing active or paused subscription with us. Please log in again and start estimating.' +
                            '<br/><br/>If you still do not see an active subscription, please contact customer service using our live chatbox, accessible from the bottom right corner of your screen.</p > ');
                 
                        $('#email').val('');
                        $('#customer_id').val('');
                        $('#payment_source_id').val('');
                        $('input#submit-button').prop("disabled", true);
                        $('input#submit-button').hide();
                        MicroModal.show('modal-1');
                    }
                    else if (data.customer != null && data.card != null) {
                    ...
                    MicroModal.init();
                    $('#modal-1 #modal-1-title').html('Subscription not found!');
                    $('#modal-1 #modal-1-content').html('<p>Your subscription Information could not be found.' +
                        '<br/><br/>Please contact customer service using our live chatbox, accessible from the bottom right corner of your screen.</p > ');
            ...
            $('#confirm-email, #email').off('change').on('change', function () {
                ...
                if (email != '' && confirmemail != '') {
                    if (email != confirmemail) {
                        ...
                        var firstName = $.trim($('#first_name').val()) == '' ? ' ' : $.trim($('#first_name').val());
                        var lastName = $.trim($('#last_name').val());
                        var cbCustomerApiUrl = cbRegApiUrl + 'getcustomer/' + encodeURIComponent(email) + '/' + encodeURIComponent(firstName) + '/' + encodeURIComponent(lastName);
        ...
        var cedomain = "https://www.clearestimates.com"; //"https://newsite.clearestimates.com";    CHANGE
        var cbInstance = Chargebee.init({
            site: "clearestimates", //"clearestimates-test", //CHANGE
            publishableKey: "live_3V5PE3cn04MqdLrPbcdoPS4yeafoxX1aK",//"test_acdJK2bp6iEgP6rOp8n6DRLyUv4v0OKcj", //CHANGE
            domain: cedomain
        });
        ...
        cbInstance.load("components").then(() => {
            ...
            $("#checkout-form").off("submit").on("submit", function(event) {  // Submitting the checkout form
                ...
                var fnameParam = encodeURIComponent($.trim(data.first_name));
                var lnameParam = $.trim(data.last_name) != '' ? encodeURIComponent($.trim(data.last_name)) : '';
                var phoneParam = $.trim(data.phone) != '' ? formatToE164(encodeURIComponent($.trim(data.phone))) : '';
                ...
                if ($('#customer_id').val() !== "" && $('#payment_source_id').val() !== "") { //It's for an existing customer and the customer already has a valid card on file
                    ...
                    UpdateBillingInfo(objSubscription.CustomerId, objSubscription.PaymentSourceId, data);
                    ...
                    ChargebeeCreateSubscription(objSubscription, cedomain, encodeURIComponent($.trim(data.email)), hasCustomerId, phoneParam, fnameParam, lnameParam);
                }
                    ...
                    cardComponent.tokenize(user).then(response => {
                        ...
                        objSubscription.AutoCollection = AutoCollectionEnum.Off;
    ...
    function ChargebeeCreateSubscription(objSubscription, cedomain, email, hasCustomerId, phone, fname, lname) {
        grecaptcha.ready(function () {
            grecaptcha.execute('6LflQdcpAAAAALgGp1Avv7Un6vWEbhNWrMaCdH7b', { action: 'submit' }).then(function (token) {
                objSubscription.RecaptchaToken = token;
                $.ajax({
                    type: 'POST',
                    ...
                    if (resp.success && resp.subscription_id != null && resp.subscription_id !== '') {
                        ...
                        if (!hasCustomerId) {
                            window.location.href = cedomain + '/subscribe/subscribe-success?registrationtoken=' + resp.subscription_id + '&email=' + email + '&phone=' + phone + '&fname=' + fname + '&lname=' + lname;
                        }
                        else {
                            var urlParams = new URLSearchParams(window.location.search.toLowerCase());
                            var returnUrl = urlParams.get('returnurl');
                            window.location.href = cedomain + '/subscribe/resubscribe-success' + (returnUrl != null && returnUrl != '' ? '?returnUrl=' + returnUrl : '');
                        }
                    }
    ...
    function formatToE164(phoneNumber, defaultCountryCode = "+1") {
        ...
        // If the number starts with '1' and is 11 digits long (North America), assume it's US/Canada
        if (cleaned.length === 11 && cleaned.startsWith("1")) {
            return `+${cleaned}`;
        }

        // If the number is 10 digits long (North America), prepend the default country code
        if (cleaned.length === 10) {
            return `${defaultCountryCode}${cleaned}`;
        }

        // If the number is longer than 10 digits and starts with a valid country code, add "+"
        if (cleaned.length > 10) {
            return `+${cleaned}`;
        }

        // Return null if the number doesn't meet valid length criteria
        return null;
    }
    function PopulateFormFields(data) {
        ...
        if (data.card.Status != null && data.card.Status == StatusEnum.Valid) {
                    ...
                    if (formData.first_name == '' || formData.last_name == '' || formData.address == '' || formData.city == '' || formData.state == '' || formData.zip == '') {
                        ...
                        if (data.customer.Id != null && data.customer.Id !== "") {
                            ...
                            cardComponent.tokenize(user).then(response => {
                                ...
                                $('#btnSavePayment').prop("disabled", false);
                                $('#customPaymentError').hide();
                                $("#card-number-error").html(error).show();
            ...
            if (data.customer.BillingAddress.State != null && data.customer.BillingAddress.State != '') {
    ...
    function UpdateBillingInfo(customerId, paymentSourceId, formData) {
