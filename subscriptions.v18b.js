...
Webflow.push(function () {
        ...
        cbInstance.load("components").then(() => {
            ...
            $("#checkout-form").off("submit").on("submit", function(event) {  // Submitting the checkout form
                ...
                if ($('#customer_id').val() !== "" && $('#payment_source_id').val() !== "") { //It's for an existing customer and the customer already has a valid card on file
                    ...
                    var nameParam = encodeURIComponent($.trim(data.first_name)) + ($.trim(data.last_name) != '' ? ' ' + encodeURIComponent($.trim(data.last_name)) : '');
                    var phoneParam = $.trim(data.phone) != '' ? formatToE164(encodeURIComponent($.trim(data.phone))) : '';
                    ChargebeeCreateSubscription(objSubscription, cedomain, encodeURIComponent($.trim(data.email)), hasCustomerId, phoneParam, nameParam);
                }
    ...
    function ChargebeeCreateSubscription(objSubscription, cedomain, email, hasCustomerId, phone, name) {
            ...
            grecaptcha.execute('6LflQdcpAAAAALgGp1Avv7Un6vWEbhNWrMaCdH7b', { action: 'submit' }).then(function (token) {
                ...
                $.ajax({
                    ...
                    if (resp.success && resp.subscription_id != null && resp.subscription_id !== '') {
                        ...
                        if (!hasCustomerId) {
                            window.location.href = cedomain + '/subscribe/success?registrationtoken=' + resp.subscription_id + '&email=' + email + '&phone=' + phone + '&name=' + name;
                        }
                        else {
