...
Webflow.push(function () {
        ...
        cbInstance.load("components").then(() => {
            ...
            $("#checkout-form").off("submit").on("submit", function(event) {  // Submitting the checkout form
                ...
                if ($('#customer_id').val() !== "" && $('#payment_source_id').val() !== "") { //It's for an existing customer and the customer already has a valid card on file
                    ...
                    ChargebeeCreateSubscription(objSubscription, cedomain, encodeURIComponent($.trim(data.email)), hasCustomerId);
                }
    ...
    function ChargebeeCreateSubscription(objSubscription, cedomain, email, hasCustomerId) {
        grecaptcha.ready(function () {
            grecaptcha.execute('6LflQdcpAAAAALgGp1Avv7Un6vWEbhNWrMaCdH7b', { action: 'submit' }).then(function (token) {
                objSubscription.RecaptchaToken = 'HFbTZofk9OEF0UaCNdQxxMQlpoEX0_TWg7bWkWJHYUZWgMC2YGMzMHaz1AYmxILFwDBmdbCUUbRANVLwsjOxQiO0EnEwB_T2JjUQQgTClGQm49BSI7VXcCcidvdARZQBZQATZgJmBNZzYwcxVqQx43bBVQZxghGDgoJgV3fkg-EF1lK3QbWh1Le0pvdGw_Vmk_bwxCSSN1O29-TmRSJEIQfDpq',//token;
                $.ajax({
                    type: 'POST',
