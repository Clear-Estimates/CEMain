        ...
        if (email.length > 0) {
            if (!regex.test(email)) {
                if ($('#email-error').length == 0) {
                    ...
                    $('<div id="email-error"><label style="padding-top:2px;color:#e96c6a;font-size: 14px;margin-bottom: 10px;">Invalid email address</label></div>').insertAfter("#email");
                }
            }
        ...
        cardComponent.tokenize(user).then(response => {
            ...
            fetch(ceproxydomain, requestOptions)
              ...
              //$("#checkout-form").hide();
                ...
                window.location.href = cedomain + "/subscribe/success?registrationtoken=" + result.subscription.id;
            }).catch(error => {
              $("#submit-button").removeClass("submit");
              $("#error").show();
