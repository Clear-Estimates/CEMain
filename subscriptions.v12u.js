  ...
  Webflow.push(function() {
    ...
    var cbdomain = "https://clearestimates-test.chargebee.com/"; //https://clearestimates.chargebee.com/
    ...
    var cbSite = "clearestimates-test"; //"clearestimates"
    var publishableCBKey = "test_NLnmWzIrJCKIUfkNfirdY7x5JUQfhmHI"; //"live_3V5PE3cn04MqdLrPbcdoPS4yeafoxX1aK"
    ...
    $('#confirm-email, #email').on('keyup change', function () {
        ...
        if (emailError) {
            ...
            console.log('no email error: Key = ' + btoa(publishableCBKey));
            $("#submit-button").prop("disabled", false);
            var cbgetcustomerurl = cbdomain + "api/v2/customers?email[is]=" + encodeURI(email) + "&sort_by[desc]=created_at&limit=1";
            var cbgetcustomersheader = new Headers();
            cbgetcustomersheader.append("Authorization", "Basic " + btoa(publishableCBKey + ':'));
            //cbgetcustomersheader.append("Content-Type", "application/json");
            //cbgetcustomersheader.append("Access-Control-Allow-Origin", "*");
            let requestOptions = { method: 'GET', headers: cbgetcustomersheader, cache: 'no-cache', mode: "no-cors", redirect: 'follow' };
            console.log('URL: ' + cbgetcustomerurl);
            fetch(cbgetcustomerurl, requestOptions)
                .then(response => //{
                    response.text()
                    //if (!response.ok) {
                    //    console.log('then => response = NOT OK : ' + JSON.stringify(response));
                    //    response.json().then(j => {
                    //        //if (j.error_param == "coupon" || j.param == "coupon") {
                    //        //    $("#coupon-error").css("display", "block");
                    //        //}
                    //        console.log('!response.ok - JSON: ' + JSON.stringify(j));
                    //    });
                    //}
                    //else {
    ...
    var cbInstance = Chargebee.init({
        site: "clearestimates-test",//cbSite,
        publishableKey: "test_sqbO05Vgy9CUM4P5jZrt53HiN0Xne8Ir",//"live_3V5PE3cn04MqdLrPbcdoPS4yeafoxX1aK",//publishableCBKey,
        domain: cedomain
    });
