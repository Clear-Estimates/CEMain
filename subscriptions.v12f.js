    ...
    cbInstance.load("components").then(() => {
      ...
      function PopulateFormFields(data) {
          ...
          if (data.card.status != null && data.card.status == "valid") {
              ...
              //$('iframe#card-number_frame').load(function () {
              var cardNumberInput = $('iframe#card-number_frame').contents().find('input#cardnumber');
              console.log($('iframe#card-number_frame').length + ' -- ' + cardNumberInput.length + ' -- ' + $.trim(cardNumberInput.val()));
                  if ($.trim(cardNumberInput.val()) == '' && data.card.masked_number != null) {
                      cardNumberInput.attr('value', data.card.masked_number);
                  }
              //});
              //$('iframe#card-expiry_frame').load(function () {
                  var cardExpDateInput = $('iframe#card-expiry_frame').contents().find('input#exp-date');
                  if ($.trim(cardExpDateInput.val()) == '' && data.card.expiry_month != null && data.card.expiry_year != null) {
                      cardExpDateInput.attr('value', data.card.expiry_month + '/' + data.card.expiry_year);
              }
              console.log($.trim(cardExpDateInput.val()));
              //});
          }
      }
  });