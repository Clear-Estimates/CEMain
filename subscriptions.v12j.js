  ...
  Webflow.push(function() {
    ...
    cbInstance.load("components").then(() => {
      ...
      function PopulateFormFields(data) {
          ...
          $('#img-card').attr('src', "https://uploads-ssl.webflow.com/6144fd968952ef8fcdb1755a/64515d2162e55df60d1370c7_visa.png");
          if (data.card.status != null && data.card.status == "valid") {
              ...
              if ($.trim($('#cardinfoblock #card-number-1').val()) == '' && data.card.masked_number != null) {
                  $('#cardinfoblock #card-number-1').val(data.card.masked_number);
                  $('#cardinfoblock #card-number-1').prop("disabled", true).css('background-color','#f8f8f8');
              }
              if ($.trim($('#cardinfoblock #card-expiry-1').val()) == '' && data.card.expiry_month != null && data.card.expiry_year != null) {
                  $('#cardinfoblock #card-expiry-1').val((data.card.expiry_month.length == 1 ? '0' + data.card.expiry_month : data.card.expiry_month ) + '/' + data.card.expiry_year);
                  $('#cardinfoblock #card-expiry-1').prop("disabled", true).css('background-color', '#f8f8f8');
              }

              $('#cardinfoblock #card-cvv-11').val('****');
              $('#cardinfoblock #card-cvv-1').prop("disabled", true).css('background-color', '#f8f8f8');
