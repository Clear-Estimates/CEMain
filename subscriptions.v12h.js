    ...
    cbInstance.load("components").then(() => {
      ...
      function PopulateFormFields(data) {
          ...
          if (data.card.status != null && data.card.status == "valid") {
              ...
              if ($('iframe#card-number_frame').length == 0 && $.trim($('input#cardnumber').val()) == '' && data.card.masked_number != null) {
                  $('input#cardnumber').val(data.card.masked_number)
              }
              if ($('iframe#card-expiry_frame').length == 0 && $.trim($('input#exp-date').val()) == '' && data.card.expiry_month != null && data.card.expiry_year != null) {
                  $('input#exp-date').val(data.card.expiry_month + '/' + data.card.expiry_year)
              }
              if (data.card.payment_source_id != null) {
                  $('#hdnPaymentSourceId').val(data.card.payment_source_id);
              }
              if (data.card.last4 != null) {
                  $('#hdnLast4').val(data.card.last4);
              }
          }
      }
  });